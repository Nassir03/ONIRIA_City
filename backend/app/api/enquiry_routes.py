...
async def list_leads(service: Annotated[LeadService, Depends(get_lead_service)]):
...
async def get_lead(lead_id: int, service: Annotated[LeadService, Depends(get_lead_service)]):
